...
- `src/core/module.ts`: 核心插件，提供通用 agents 与 tools。
- `src/board/index.ts`: board 插件，提供 board 专用 agents 与 tools。
- `src/app/plugins.ts`: 应用层选择要装配的插件集合。
- `src/app/skill-plugin.ts`: 应用层通过 `RuntimePlugin.skills` 声明具体 skill 内容。
- `src/core/plugin/manager.ts`: 负责注册 plugin 的 `agents` / `tools` / `skills`，并执行可选 `setup` / `dispose` 生命周期。
...
- 新增 runtime plugin: 新建插件对象后注册到 `src/app/plugins.ts`
